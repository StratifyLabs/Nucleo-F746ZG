/* Do not modifiy this file.
 * It was generated using the sl command line tool
 * Change the settings using the tool then build again using sl
 */

#ifndef SL_CONFIG_H_
#define SL_CONFIG_H_

#define SL_CONFIG_VERSION_STRING "0.9"
#define SL_CONFIG_VERSION_BCD 0x09
#define SL_CONFIG_DOCUMENT_ID "XCw1jMjiObLMUDKZFRG2"
#define SL_CONFIG_NAME "Nucleo-F746ZG"
#define SL_CONFIG_TYPE "os"
#define SL_CONFIG_PUBLISHER "Stratify Labs, Inc"
#define SL_CONFIG_PERMISSIONS "public"
#define SL_CONFIG_HARDWARE_ID_STRING "0x00000013"

#endif

